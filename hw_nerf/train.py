...
def train(rawData, model, optimizer, n_iters=3000):
    ...
    for i in tqdm(range(n_iters)):
        ...
        # Compute the rays for the selected image.
        rays_o, rays_d = get_rays(H, W, focal, pose)  # rays_o and rays_d shapes: [H, W, 3]

        # Flatten the rays to shape [H*W, 3] for processing.
        rays_o = rays_o.view(-1, 3)  # Shape: [H*W, 3]
        rays_d = rays_d.view(-1, 3)  # Shape: [H*W, 3]

        # Render the scene using the current model state.
        rgb, depth = render(model, rays_o, rays_d, near=2.0, far=6.0, n_samples=n_samples, rand=True)  # rgb shape: [H*W, 3]

        # Zero the gradients.
        optimizer.zero_grad()

        # Compute the loss between the rendered image and the target image.
        # Flatten the target image to match the shape of rgb.
        image_loss = F.mse_loss(rgb, target.view(-1, 3))
...
def main():
    ...
    with torch.no_grad():
        ...
        rays_o = rays_o.view(-1, 3)  # Shape: [H*W, 3]
        rays_d = rays_d.view(-1, 3)  # Shape: [H*W, 3]
        rgb, depth = render(nerf, rays_o, rays_d, near=2.0, far=6.0, n_samples=64, rand=False)
