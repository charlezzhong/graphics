...
def get_rays(H, W, focal, pose):
    ...
    x = (u - W * 0.5) / focal
    # Normalize y coordinates, and invert it
    y = -(v - H * 0.5) / focal  
    # Negative z-axis with all -1 (focal length / focal length)
    z = -torch.ones_like(x)  

    # [H, W, 3]
    dirs = torch.stack([x, y, z], dim=-1)  
    #dirs = torch.stack([(u - W * 0.5) / focal, -(v - H * 0.5) / focal, -torch.ones_like(u)], -1)
    ...
    # Extract rotation matrix (first 3 columns of pose)
    rotation = pose[:3, :3]

    # Transform dirs to world coordinates
...
def positional_encoder(p, L_embed=6):
    ...
    for i in range(L_embed):
        ...
        # (p, sin(2^0 * p), cos(2^0 * p), ..., sin(2^i * p), cos(2^i * p))
        for fn in [torch.sin, torch.cos]:
            rets.append(fn((2.0 ** i) * p))
...
def render(model, rays_o, rays_d, near, far, n_samples, rand=False):
    ...
    #rays_o_expand = rays_o[:, None, :]  # [N_rays, 1, 3]
    #rays_d_expand = rays_d[:, None, :]  # [N_rays, 1, 3]
    #t_expand = t[..., None]             # [N_rays, n_samples, 1]
    #points = rays_o_expand + rays_d_expand * t_expand  # [N_rays, n_samples, 3]
    rays_o_expand = rays_o.unsqueeze(1)  # Shape: [N_rays, 1, 3]
    rays_d_expand = rays_d.unsqueeze(1)  # Shape: [N_rays, 1, 3]
    t_expand = t.unsqueeze(0).unsqueeze(2) # Shape: [1, n_samples, 1]
    points = rays_o_expand + rays_d_expand * t_expand  # Shape: [N_rays, n_samples, 3]
